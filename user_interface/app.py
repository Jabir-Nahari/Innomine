...
def show_ds18b20():
    rows = fetch_recent_ds18b20(limit=limit)
    df = pd.DataFrame(rows)
    if df.empty:
...
def show_mpu6050():
    rows = fetch_recent_mpu6050(limit=limit)
    df = pd.DataFrame(rows)
    if df.empty:
    ...
    col1, col2 = st.columns(2)
    with col1:
        axis = st.selectbox("Accel axis", ["accel_x_g", "accel_y_g", "accel_z_g"], index=0)
        _line_chart(df, "recorded_at", axis, f"Acceleration ({axis})")

    with col2:
        axis = st.selectbox("Gyro axis", ["gyro_x_dps", "gyro_y_dps", "gyro_z_dps"], index=0)
        _line_chart(df, "recorded_at", axis, f"Gyroscope ({axis})")

    unit = st.selectbox("MPU temperature unit", ["temperature_c", "temperature_f"], index=0)
...
def show_alarms():
    rows = fetch_recent_alarms(limit=min(limit, 500))
    df = pd.DataFrame(rows)
    if df.empty:
    ...
    if "metric" in df.columns:
        metric = st.selectbox("Metric", sorted(df["metric"].dropna().unique().tolist()))
        ...
        if not sub.empty and "value" in sub.columns:
            c = (
                ...
                .properties(title=f"Alarms: {metric}", height=280)
            )
            st.altair_chart(c, use_container_width=True)
